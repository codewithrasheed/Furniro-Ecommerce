    ...
    <>
          ...
          <div className="grid items-start grid-cols-1 lg:grid-cols-2 gap-6 max-lg:gap-12">
            ...
            <div className="px-4 sm:px-10">
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-4 mt-4">
                  {['#816DFA', '#000', '#B88E2F'].map((color) => (
                    <button
                      key={color}
                      type="button"
                      className={`w-[30px] h-[30px] bg-[${color}] rounded-full flex items-center justify-center shrink-0`}
                    ></button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-x-5 w-full sm:w-[500px]">
                <div className="w-full sm:w-[130px] h-[64px] mt-8 px-6 font-sans py-3 bg-transparent border border-[#9F9F9F] text-sm font-medium rounded-xl flex items-center justify-between gap-4">
                ...
                <p className="flex mb-2 text-[#9F9F9F]">
                  ...
                  <span className="px-10 flex items-center justify-center gap-2">
                    ...
                    {[fb, linkedin, twitter].map((icon, index) => (
                      <span key={index}>
                        <Image src={icon} width={20} height={20} alt={`social-icon-${index}`} />
                      </span>
                    ))}
                  </span>
                </p>
      ...
      <div className="border-t-[1px] border-t-[#9F9F9F] mb-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-5 px-4">
          <Image src={seater1} width={570} alt="specification" className="w-full sm:w-auto" />
          <Image src={seater2} width={570} alt="specification" className="w-full sm:w-auto" />
        </div>
      </div>
      <div className="border-t-[1px] border-t-[#9F9F9F] mb-10">
        <div className="heading flex justify-center items-center gap-4 mt-10 mb-10">
          <h1 className="font-semibold text-2xl sm:text-[36px]">Related Products</h1>
        </div>
        <div className="container px-5 py-10 mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {[
            { image: cart1, label: label1, title: "Syltherine", description: "Stylish cafe chair", price: "Rp 2.500.000", oldPrice: "3.500.000" },
            { image: cart2, title: "Leviosa", description: "Stylish cafe chair", price: "Rp 2.500.000" },
            { image: cart3, label: label2, title: "Lolito", description: "Luxury big sofa", price: "Rp 7.000.000", oldPrice: "14.000.000" },
            { image: cart4, label: label3, title: "Respira", description: "Outdoor bar table and stool", price: "Rp 500.000" }
          ].map((product, index) => (
            <div
              key={index}
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="mt-1">
                  {product.price} &nbsp;{" "}
                  {product.oldPrice && <del className="text-gray-400">{product.oldPrice}</del>}
                </p>
              </div>
            </div>
          ))}
